...
pdf_reader = PdfReader("files/COS324_Course_Notes.pdf")

## Find the page number where the Table of Contents starts
toc_page_no = 0
while toc_page_no < len(pdf_reader.pages):
    lines = pdf_reader.pages[toc_page_no].extract_text().split('\n')
    if lines[0] == 'Contents':
        break
    toc_page_no += 1
...
page_split_markers[-1].append(len(pdf_reader.pages) + 1)
